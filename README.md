...
- Given a training set of $\large n$ input mixture signals and $\large m$ target sources per mixture, $\large D = \set{(A_{i}, T_{i}): i = 1,\dots, n, T_{i} = (t_{i}^1,...,t_{i}^k)}$, we pre-process each pair by:
