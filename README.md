...
```
#### 2-Component Loss:
$\Huge L_{2c}(X; Y_{k}; \theta; \alpha) = \frac{1-\alpha}{n} ||Y_{filter, k} - |Y_{k}|||^{2}_{2}+\frac{\alpha}{n}||ddfR_{filter}sdf||^{2}_{2}dfdf$

where:


$\Huge Y_{k, filter} = M_{\theta} \odot |Y_{k}| + \frac{\alpha}{n}|| R_{filter}||^{2}_{2}$
