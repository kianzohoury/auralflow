...
- Given a training set of $\large n$ mixture-target audio pairs, $\large D = \set{(A_{i}, T_{i}): i = 1,\dots, n}$, where T_{i}^k corresponds to target source $\large k$ in , T_{i} = (t_{i}^1,...,t_{i}^m), we pre-process each pair by:
