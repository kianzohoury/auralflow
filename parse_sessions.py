...
def model_config_handler(**config_args):
    ...
    try:
        ...
        print(f"Success: saved model configuration file for {model_name}",
    ...
    try:
        ...
        print(f"Success: saved audio data configuration file for {model_name}",
              f"({base_model}) to {session_id}.")
    except YAMLError as e:
        print(str(e))
        sys.exit(1)
    try:
        ...
        print(f"Success: saved training settings configuration file for",
...
def new_session_handler(**session_args):
    ...
    if not save_dir.is_dir():
        print(f"FileNotFoundError: no such parent directory:",
              f"{save_dir.absolute()}")
    else:
        ...
        try:
            ...
            print(f"FileExistsError: session already exists:",
                  f"{session_dir.name}")
            sys.exit(1)
        try:
            session_data = OrderedDict({
                ...
                'date-created': datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                'models': []
            })
